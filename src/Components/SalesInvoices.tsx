...
const SalesInvoices: React.FC = () => {
  ...
  const loadSalesInvoice = async () => {
    try {
      ...
      if (data.salesInvoiceDate) {
        const tempDate = new Date(data.salesInvoiceDate);
      ...
      setExpireDate(
        data.expireDate
          ...
          : addOneMonth(new Date(data.salesInvoiceDate)),
      );
  ...
  useEffect(() => {
    const loadVatRates = async () => {
      try {
        const res = await api.get("/api/commondropdown/loadVatRateDropdown");
  ...
  useEffect(() => {
    const loadLineItemDropdown = async () => {
      try {
        const res = await api.get("/api/commondropdown/loadLineItemDropdown");
  ...
  useEffect(() => {
    const loadCustomerDropdown = async () => {
      try {
        const res = await api.get("/api/commondropdown/loadCustomerDropdown");
  ...
  useEffect(() => {
    const fetchSalesInvoiceNo = async () => {
      try {
        const res = await api.get("/api/salesinvoices/getNextSalesInvoiceNo");
  ...
  useEffect(() => {
    const loadSalesOrders = async () => {
      try {
        const res = await api.get("/api/salesinvoices/salesOrderDropdown");
  ...
  const isItemAlreadyAdded = (itemID: number, currentRowId: number) => {
    return rows.some((r) => r.itemID === itemID && r.rowId !== currentRowId);
  };
  ...
  const onQuantityChange = (rowId: number, value: string) => {
    setRows((prev) =>
      prev.map((row) =>
        row.rowId === rowId ? recalculateRow({ ...row, quantity: value }) : row,
      ),
    );
  };
  ...
  const submitSalesInvoice = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        res = await api.put(
          `/api/salesinvoices/updateSalesInvoice/${salesInvoiceID}`,
          payload,
        );
    ...
    <div className="p-6 bg-white rounded shadow">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative w-[210px]">
            <DatePicker
              ...
              minDate={addOneMonth(salesInvoiceDate)}
              openToDate={addOneMonth(salesInvoiceDate)}
              dateFormat="yyyy-MM-dd"
              popperPlacement="bottom-start"
              popperClassName="z-50"
              className="w-full h-[28px] px-2 rounded border border-gray-400 text-gray-700 text-[12px]"
      ...
      <table className="w-full text-[10px] border border-gray-400 rounded rounded-lg">
        <thead className="bg-[#1c3c61] text-[10px] leading-tight">
          <tr>
            ...
            <th className="p-2 text-center text-white text-[10px]">
              Unit Price
            </th>
            ...
            <th className="p-2 w-20 text-center text-white text-[10px]">
              Action
            </th>
          </tr>
        </thead>
              ...
              <td className="p-1 w-40 text-xxs font-lato">
                <div className="relative w-full">
                  <select
                    value={row.itemID ?? ""}
                    onChange={(e) =>
                      onItemChange(row.rowId, Number(e.target.value))
                    }
                    className="w-full text-[10px] text-gray-800 h-[28px] px-2 pr-8 rounded border border-gray-400 appearance-none"
                  >
              ...
              <td className="p-1 w-32">
                <div className="relative w-full">
                  <select
                    value={row.vatRateID}
                    onChange={(e) =>
                      onVatChange(row.rowId, Number(e.target.value))
                    }
      ...
      <div className="mt-4 grid grid-cols-6 gap-2">
        ...
        <div className="col-span-1 flex items-center gap-2">
          <label className="w-30 text-right text-[10px]">Total Discount</label>
          <input
            type="text"
            readOnly
        ...
        <div className="col-span-1 flex items-center gap-2">
          <label className="w-30 text-right text-[10px]">Excl. Amount</label>
          <input
            type="text"
            readOnly
        ...
        <div className="col-span-1 flex items-center gap-2">
          <label className="w-20 text-right text-[10px]">Total VAT</label>
          <input
            type="text"
            readOnly
