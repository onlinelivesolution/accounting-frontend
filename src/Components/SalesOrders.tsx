...
const SalesOrders: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSalesOrderNo = async () => {
      try {
        const res = await api.get("/api/salesorders/getNextSalesOrderNo");
...
//           "http://127.0.0.1:8000/api/salesinvoices/salesOrderDropdown",
//         );
...
//   }, []);
  ...
  const submitSalesOrder = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        res = await api.put(
          `/api/salesorders/updateSalesOrder/${salesOrderID}`,
          payload,
        );
